"""Algothon 2026 submission (v6) — PURE aggregated lead-lag, bang-bang sized.

    getMyPosition(prcSoFar) -> length-51 vector of integer share positions.

v6 = v2 with the reversal sleeve REMOVED (REV_W=0). The ONLY change vs v2. Rationale: two live
751-1000 results proved the reversal sleeve is unstable and currently harmful — v4 (rev_w=0.15)
scored 841.13, v5 (rev_w=0.25 + a 60d horizon) scored 671.85 on the same window, i.e. MORE
reversal -> LOWER live score — while offline (501-750) more reversal looked BETTER. The lead-lag
core is the only component that generalises from backtest to live, so we lean on it alone.
Live anchor to beat: v4 = 841.13. See CLAUDE.md "GENERAL ROUND" for the full evidence.
...
REV_W = 0.0             # reversal REMOVED. Two live 751-1000 results proved the reversal sleeve
                        # is an unstable coin-flip: v4 (rev_w=0.15) scored 841.13, v5 (rev_w=0.25 +
                        # a 60d horizon) scored 671.85 on the SAME window. More reversal -> worse
                        # live, even though more reversal was BETTER on the 501-750 backtest (v5 606
                        # vs v4 554). The sleeve's sign flips between adjacent windows, so offline
                        # tuning of it is anti-predictive. Only the lead-lag core is stable; keep it
                        # alone. (rev_w=0 makes REV_LB irrelevant.)
REV_LB = 20             # unused when REV_W=0
